...
- **视频广告（片头和片中）无法被内容拦截器拦截。** 因此才有 `视频广告跳过` 网页扩展，它分两层工作：
  - `WebExtension/main.js`（主世界，Safari 16.4 或更高）：从播放器响应中删除广告条目，使广告根本不会开始。对象包括页面内嵌的 `ytInitialPlayerResponse`，以及页面内导航时经 `JSON.parse`、`Response.json` 解析的所有响应。没有延迟。同时删除随该响应一起下发的 YouTube 广告拦截器警告，因为该警告会暂停视频，使其停在起始画面。
  - `WebExtension/content.js`（隔离世界）：如果广告仍然出现（例如服务器端插入的广告），点击跳过按钮或跳转到广告结尾。这条路径要先加载广告再结束它，因此会残留 1 到 3 秒的延迟。如果警告对话框仍然出现，会在其出现的瞬间被移除并恢复播放。诊断记录写入控制台和页面 `<html>` 元素的 `data-sab-log`。
