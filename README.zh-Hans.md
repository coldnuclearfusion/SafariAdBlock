...
- **视频广告（片头和片中）无法被内容拦截器拦截。** 因此才有 `视频广告跳过` 网页扩展，它分两层工作：
  - `WebExtension/main.js`（主世界，Safari 16.4 或更高）：从播放器响应中删除广告条目，使广告根本不会开始。对象包括页面内嵌的 `ytInitialPlayerResponse`，以及页面内导航时经 `JSON.parse`、`Response.json` 解析的所有响应。没有延迟。随该响应一起下发的 YouTube 广告拦截器警告则有意保留：删除它会让播放器显示一般性错误画面。
