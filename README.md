...
- **Video ads (pre-roll and mid-roll) cannot be blocked by content blockers.** That is why the `Video Ad Skipper` web extension exists, with two layers:
  - `WebExtension/main.js` (main world, Safari 16.4 or later): removes the ad entries from the player response — the `ytInitialPlayerResponse` embedded in the page and every response parsed with `JSON.parse` or `Response.json` during in-page navigation — so ads never start. No delay. It also removes YouTube's ad-blocker warning from the same response, because that warning pauses the video and would otherwise leave it stopped at the start.
  - `WebExtension/content.js` (isolated world): if an ad still appears (server-side inserted ads, for example), clicks the skip button or seeks to the end of the ad. This path loads the ad first and then ends it, so a delay of one to three seconds remains. If the warning dialog still shows up, it is removed the moment it appears and playback is resumed. Diagnostics are written to the console and to `data-sab-log` on the page's `<html>` element.
