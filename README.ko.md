...
- **영상 광고(재생 전·중간)는 콘텐츠 차단기로는 못 막습니다.** 그래서 `동영상 광고 건너뛰기` 웹 확장이 따로 있고, 두 겹으로 동작합니다.
  - `WebExtension/main.js` (메인 월드, Safari 16.4 이상): 플레이어가 받는 응답(`/youtubei/v1/player`, 페이지에 박힌 `ytInitialPlayerResponse`)에서 광고 항목을 지워 광고가 아예 시작되지 않게 합니다. 지연이 없습니다.
  - `WebExtension/content.js` (격리 월드): 그래도 광고가 나오면(서버 삽입형 등) 건너뛰기 버튼을 누르거나 광고 영상을 끝으로 돌립니다. 이 경로는 광고를 한 번 로드했다 끝내는 방식이라 1~3초 지연이 남습니다.
